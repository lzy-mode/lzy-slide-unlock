...
const SlideUnlock = defineComponent({
  name: "SlideUnlock",
  props: {
    text: {
      type: String,
  ...
  setup(props, { attrs, emit, expose }) {
    ...
    const stopDrag = () => {
      ...
      if (percent() >= props.successThreshold) {
        complete();
      } else {
        update(0);
      }
    ...
    const onKeydown = (event) => {
      ...
      if (event.key === "Enter" && percent() >= props.successThreshold) {
        complete();
      }
    };

    nextTick(() => update(0, false));

    watch(
      () => [props.width, props.height],
      () => {
        nextTick(() => update(value.value, false));
      }
    );
      ...
      h(
        ...
        [
          h("div", {
            ...
            style: {
              width: `${value.value + (handleRef.value?.offsetWidth ?? 0) + 8}px`
            }
          }),
          h("div", { class: "slide-unlock__text" }, props.text),
          h("button", {
            ref: handleRef,
